...
//    printQuakes();
  ...
  private void printQuakes() {
    ...
    HashMap<String, ArrayList<String>> countriesQuakes = new HashMap<>();
    int oceanQuakesNum = 0;
    //Loop through all quake markers
    for (Marker qm : quakeMarkers) {
      EarthquakeMarker em = (EarthquakeMarker) qm;
      if (em.isOnLand()) {
        String countryName = (String) qm.getProperty("country");
        if (!countriesQuakes.containsKey(countryName)) {
          ArrayList<String> quakeTitles = new ArrayList<>();
          quakeTitles.add(em.getTitle());
          countriesQuakes.put(countryName, quakeTitles);
        } else {
          countriesQuakes.get(countryName).add(em.getTitle());
        }
      } else {
        oceanQuakesNum++;
      }
    ...
    for (String countryName : countriesQuakes.keySet()) {
      int numOfQuakes = countriesQuakes.get(countryName).size();
