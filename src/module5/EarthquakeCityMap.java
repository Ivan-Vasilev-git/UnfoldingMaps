...
//    printQuakes();
  ...
  private boolean inThreatCircle(Marker marker) {
    if (lastClicked instanceof EarthquakeMarker && marker instanceof CityMarker) {
      if (lastClicked.getDistanceTo(marker.getLocation()) <= ((EarthquakeMarker) lastClicked).threatCircle()) {
        return true;
      }
    }
    if (lastClicked instanceof CityMarker && marker instanceof EarthquakeMarker) {
  ...
  private void hideMarkers() {
    for (Marker marker : quakeMarkers) {
      if (!lastClicked.equals(marker) && !inThreatCircle(marker)) {
